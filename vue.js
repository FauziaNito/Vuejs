var Vue = (function (exports) {
  ...
  */
	...
	function setRef(rawRef, oldRawRef, parentSuspense, vnode, isUnmount = false) {
		...
		const refValue =
			vnode.shapeFlag & 4 /* ShapeFlags.STATEFUL_COMPONENT */ ? getExposeProxy(vnode.component) || vnode.component.proxy : vnode.el;
					...
					if (domType === 1 /* DOMNodeTypes.ELEMENT */ || domType === 3 /* DOMNodeTypes.TEXT */) {
						...
						for (let i = 0; i < vnode.staticCount; i++) {
							if (needToAdoptContent)
								vnode.children += nextNode.nodeType === 1 /* DOMNodeTypes.ELEMENT */ ? nextNode.outerHTML : nextNode.data;
	...
	const transformFor = createStructuralDirectiveTransform("for", (node, dir, context) => {
		...
		return processFor(node, dir, context, (forNode) => {
			...
			const keyExp =
				keyProp && (keyProp.type === 6 /* NodeTypes.ATTRIBUTE */ ? createSimpleExpression(keyProp.value.content, true) : keyProp.exp);
			const keyProperty = keyProp ? createObjectProperty(`key`, keyExp) : null;
			const isStableFragment =
				forNode.source.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */ && forNode.source.constType > 0; /* ConstantTypes.NOT_CONSTANT */
		...
		if (!onComponentSlot) {
			...
			if (!hasTemplateSlots) {
				...
				if (hasNamedDefaultSlot) {
					context.onError(
						createCompilerError(39 /* ErrorCodes.X_V_SLOT_EXTRANEOUS_DEFAULT_SLOT_CHILDREN */, implicitDefaultChildren[0].loc)
					);
				} else {
	...
	function resolveComponentType(node, context, ssr = false) {
		...
		if (isProp) {
			if (isExplicitDynamic || false) {
				const exp =
					isProp.type === 6 /* NodeTypes.ATTRIBUTE */ ? isProp.value && createSimpleExpression(isProp.value.content, true) : isProp.exp;
				if (exp) {
		...
		const analyzePatchFlag = ({ key, value }) => {
			if (isStaticExp(key)) {
				...
				if (
					...
					((value.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */ || value.type === 8) /* NodeTypes.COMPOUND_EXPRESSION */ &&
			...
			if (prop.type === 6 /* NodeTypes.ATTRIBUTE */) {
				...
				if (!arg && (isVBind || isVOn)) {
					...
					if (exp) {
						...
						context.onError(
							createCompilerError(isVBind ? 34 /* ErrorCodes.X_V_BIND_NO_EXPRESSION */ : 35 /* ErrorCodes.X_V_ON_NO_EXPRESSION */, loc)
						);
					}
					continue;
				}
	...
	const transformModel = (dir, node, context) => {
		...
		if (dir.modifiers.length && node.tagType === 1 /* ElementTypes.COMPONENT */) {
			...
			const modifiersKey = arg
				...
				: `modelModifiers`;
			props.push(
				createObjectProperty(modifiersKey, createSimpleExpression(`{ ${modifiers} }`, false, dir.loc, 2 /* ConstantTypes.CAN_HOIST */))
			);
		}
